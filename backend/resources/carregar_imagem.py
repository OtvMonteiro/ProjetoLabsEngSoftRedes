...
# images_id_counter = 0
# imagens_por_municipio = {'Barueri':[],
#                          'Lorena': []}

# class Imagem:
#     def __init__(self, image_id = 0, image_data = None, digitado = False, id_digitador = None) -> None:

#         self.image_id = image_id
#         self.image_data = image_data
#         self.digitado = digitado
#         self.id_digitador = id_digitador
...
class UploadForm(Resource):
    ...
    def post(cls):
        ...
        if len(request.files) == 0:
            return {'status': 'Nenhuma imagem selecionada'}

        if len(request.form['municipio']) == '':
            return {'status': 'Nenhum município selecionado'}
        ...
        # global images_id_counter, imagens_por_municipio
        # images_id_counter += 1
        # img = Imagem(image_id=images_id_counter, image_data=base64.b64encode(img_byte_arr), digitado=False, id_digitador=None)

        # imagens_por_municipio[municipio].append(img) 
        # print(imagens_por_municipio[municipio])
